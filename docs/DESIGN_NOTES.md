...
### 実装済み(2026-09-09・batch 24。全構成一致判定 `mode: 'all'`)

- **意味論(ユーザーと認識合わせ済み)**: 行は全構成に存在し、全構成でフィールドが一致するときだけ same。1 構成でも違えば、どのペインでもその行は差分になる(基準対各構成では「案1 は基準と一致 → same」だった行が、案2 と違えば 案1 のペインでも field-diff)。内訳は「自分から見た他の各構成」。
- **差分型は変えていない**(設計どおり `missingIn` / `counterparts` / `bySide` が構成 ID 引きだったため)。`isBase` は全構成 false、`baseId` は `undefined`(型を `baseId?:` に広げた)。
- **`axisId` を結果に追加**し、整列と差分ジャンプは `baseId` ではなく `axisId`(base モードでは基準、all モードでは先頭の構成)を軸にするよう置き換えた。`UseMultiComparisonNavigationOptions.comparison` の Pick も `'sides' | 'axisId'` に変更(フックの戻り値をそのまま渡す限り影響なし)。
- **判定は compare() を使わず直接計算**(各構成を 1 回索引し、各行を他の全構成と突き合わせる)。2-way の `compare()` は `equals` を (左, 右) で呼ぶため、all モードの規則「(自分の値, 相手の値)」と食い違うのを避けた。`bySide` の 2-way 差分は自側 = left の注釈として手で組む(`left-only` = その構成に無い)。`pairs` は all モードでは空 Map。
- **既定ラベル**: all モードの各ペインは基準ペインと同じ「構成別内訳の連結」(相手が 1 構成なら接頭辞なし)。`only` は `sideOnly`(この構成のみ)。`MultiDiffLabelContext` に `mode` を追加。
...
### 実装済み(2026-09-11・batch 33。横スクロール同期の明示指定 `enableHorizontalScrollSync`)

- 横同期自体は batch 17 / 18 から `useComparisonScrollSyncGroup({ syncHorizontal })` にあり、`ComparisonView` / `ComparisonLayout.Root` が layout で軸を決め打ちしていた(横並び = 縦のみ / 縦並び = 縦横)。ユーザーから「横並びでも横同期したい」の確認があり、`enableHorizontalScrollSync?: boolean` を追加(未指定 = 従来どおり layout 依存 / true = 横並びでも横同期 / false = 縦並びでも縦のみ)。`scrollSyncGroup` 注入時は無視(グループ自身の設定が勝つ。従来どおり)。
- 前提として文書化: 両ペインの列構成 / 列幅が同じときに意味がある(`enableColumnResize` で片側だけ列幅を変えると横位置はずれて見える。同期自体は動く)。
- テスト 2 件(横並び + true で横も伝わる / 縦並び + false で縦のみ)。デモ(2 構成 / N 構成)に「横も同期」トグル(スクロール同期 OFF・縦並びでは無効)。
