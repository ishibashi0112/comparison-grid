...
- **Aligned mode (`alignRows`)** — both panes get the same length in match order, with placeholder rows (styled `.cmpg-row-placeholder`) inserted on the missing side; the diff-only filter works per pair so alignment is preserved. Placeholder rows are ordinary rows to the grid, so they are copied as blank lines by default; opt in to `excludePlaceholderRowsOnCopy` to drop them from `Ctrl/⌘+C`, `exportCsv` and `getExportData` (spreadsheet-grid ≥ 0.33.0 `isRowExportable`).
...
compareMany(sides, { ...opts, mode: 'all' }); // no base: a row is same only when every side has it and agrees
```
...
- **左右整列モード(`alignRows`)** — 両ペインを突き合わせ順の同じ長さに揃え、欠損側へプレースホルダ行(`.cmpg-row-placeholder`)を挿入。「差分のみ」は対の単位でフィルタされ、整列が保たれます。プレースホルダ行はグリッドから見ると通常の行なので既定では空行としてコピーされます。`excludePlaceholderRowsOnCopy` を付けると `Ctrl/⌘+C` / `exportCsv` / `getExportData` から除かれます(spreadsheet-grid ≥ 0.33.0 の `isRowExportable`)。
...
- **スクロール同期(`enableScrollSync`)** — 両ペインのスクロールを同期(ユーザー操作のみ伝播し、API 由来は無視してループを防止)。`alignRows` との併用を想定。同期する軸はレイアウトに追従: 横並びは縦のみ、縦並び(`layout='vertical'`)は列も上下に揃うため縦横両方。`enableHorizontalScrollSync` で明示できます(横並びでも横同期したいときに `true`。両ペインの列幅が同じことが前提)。
...
compareMany(sides, { ...opts, mode: 'all' }); // 基準なし: 全構成に存在し全構成で一致する行だけが same
```
